...
echo \$name1
echo \$t_nod1
oeomega pose -in $file_path/$protein/iteration_$iteration/smile/\$tmp1\_smiles_final_updated/\$tmp1\_smiles_final_updated_\$PBS_ARRAY_INDEX.smi -out $file_path/$protein/iteration_$iteration/sdf/\$name1\_\$PBS_ARRAY_INDEX\_sdf.oeb.gz -strictstereo false -log \$name1.log -prefix \$name1\_\$PBS_ARRAY_INDEX">> $name'_'conf.sh

   qsub -N $PBS_JOBNAME -l select=1:ncpus=$t_node:mem=$t_mem:cpu_type=ivy -v f_1="$f",name1="$name",t_nod1="$t_nod",tmp1="$tmp" $name'_'conf.sh
done
wait
