...
def wait_phase_4(project, project_path):
    ...
    while not created:
        ...
        print("simple_job created. Starting to check PBS files...") 
    ...
    while not initiated:
        time.sleep(30)
        initiated = True
        if len(glob.glob(f"{simple_path}/phase_4.o*")) != len(glob.glob(f"{simple_path}/*sh")):
            initiated = False
    else:
    ...
    while not finished:
        ...
        time.sleep(30)
        finished = True
        for PBS_out_file in glob.glob(f"{simple_path}/phase_4.o*"):
            # Read the file
            with open(PBS_out_file, "r") as f:
...
def add_train_num_mol(project, project_path, training_size):
    ...
    # Add train_num_mol to each PBS script
